    ...
    <div class="bg-gradient-to-r from-blue-600 to-purple-600 dark:from-blue-800 dark:to-purple-800 rounded-xl sm:rounded-2xl shadow-lg p-6 sm:p-8 mb-6 text-white">
        <div class="flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-6">
            ...
            <div class="flex-1 text-center sm:text-left">
                ...
                <p class="text-blue-100 dark:text-blue-200 mb-4 text-sm sm:text-base break-all">{{ $user->email }}</p>
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-3 p-3 sm:p-4 bg-gray-50 dark:bg-gray-900 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition group">
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400">
                                    ...
                                    @if($post->thread)
                                        • in <a href="{{ route('threads.show', $post->thread) }}" class="text-blue-600 dark:text-blue-400 hover:underline">{{ Str::limit($post->thread->title, 30) }}</a>
                                    @endif
                                </p>
                            </div>
        ...
        @if($activities->count() > 0)
            <div class="space-y-3">
                @foreach($activities as $activity)
                    ...
                    @else
                        {{-- Post Activity (on a déjà filtré, donc thread existe forcément) --}}
                        <a href="{{ route('threads.show', $activity->thread) }}" class="flex items-start gap-3 p-3 bg-gray-50 dark:bg-gray-900 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition group">
